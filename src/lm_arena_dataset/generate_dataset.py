...
PRICE_CACHE_FILE = "src/lm_arena_dataset/openrouter_prices.json"
...
def get_openrouter_prices(use_cache=True):
    ...
    try:
        ...
        for model in models_data:
            model_id = model['id']
            prices[model_id] = {
                'input_cost_per_token': float(model.get('pricing', {}).get('prompt', 0.0)) / 1_000_000,
                'output_cost_per_token': float(model.get('pricing', {}).get('completion', 0.0)) / 1_000_000,
            }
...
def process_row(row, prices, model_mapping):
    ...
    if model_a_id not in model_mapping:
        raise ValueError(f"Model '{model_a_id}' not found in model_mapping. Please add it.")
    if model_b_id not in model_mapping:
        raise ValueError(f"Model '{model_b_id}' not found in model_mapping. Please add it.")
    ...
    if openrouter_model_a not in prices:
        raise ValueError(f"OpenRouter model '{openrouter_model_a}' (mapped from '{model_a_id}') not found in prices. Please update the mapping.")
    if openrouter_model_b not in prices:
        raise ValueError(f"OpenRouter model '{openrouter_model_b}' (mapped from '{model_b_id}') not found in prices. Please update the mapping.")
...
def main():
    ...
    output_file = "src/lm_arena_dataset/router_bench_dataset.jsonl"
